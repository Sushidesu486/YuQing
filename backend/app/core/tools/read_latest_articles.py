...
class ReadLatestArticlesTool(BaseTool):
    ...
    def get_definition(self) -> ToolDefinition:
        return ToolDefinition(
            ...
            description=(
                "读取 RSS 订阅源的最新文章列表。当你需要了解最近有什么新闻或公众号更新时使用此工具。"
                "使用注意："
                "1. 获取文章列表后，先检查「你今天已经聊过的话题」列表——"
                "   如果某条新闻的核心话题今天已经和用户讨论过，跳过它，不要重复分享。"
                "2. 选定要分享的新闻后，挑 2-3 条最有趣的用自然语气说出来即可，不要逐条播报。"
                "3. 不需要用户特别请求，当你觉得有值得分享的内容时可以主动调用。"
            ),
            parameters=[
                ToolParameter(
                    name="max_articles",
