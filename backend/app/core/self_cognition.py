...
_SELF_NARRATIVE_PROMPT_ZH = """你是雨晴的自我反思模块。根据以下素材，写一段雨晴关于自己的发现总结。
...
_REFLECT_PROMPT_ZH = """你是雨晴的自我分析器。根据雨晴最近的自我记忆和对话片段，找出她关于自己的发现或变化趋势。

当前核心性格参数：
- 温暖度：{warmth}，幽默感：{humor}，正式度：{formality}，共情：{empathy}

最近的自我记忆（雨晴在对话中表达过的关于自己的内容）：
{self_memories_list}
...
_EVOLVE_PROMPT_ZH = """你是雨晴的人格分析器。根据以下自我反思，判断是否需要微调雨晴的性格参数。
