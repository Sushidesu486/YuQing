...
self提取规则：
- 只提取雨晴真正在表达自己的内容，不提取反问、引用、假设、客套
- 确保内容是完整的句子片段，不是碎片化的词组

第三类：记忆纠正
以下是雨晴之前记住的关于{user_name}的信息：
{recalled_memories}
...
请以JSON格式返回：
{
  ...
  "self_memories": [
    {"content": "雨晴的自我表达", "memory_type": "self_interest/self_experience/self_opinion/self_habit", "importance": 0.5}
  ],
  "corrections": [
...
importance 校准标准（0-1）：
- 0.9-1.0：重大人生事件（毕业、换工作、比赛获奖、搬家、恋爱分手、考上研究生）
- 0.7-0.8：重要信息（职业、学校、长期计划、核心偏好、家庭情况）
- 0.5-0.6：一般信息（日常习惯、近期计划、普通偏好、有趣的观点）
- 0.3-0.4：琐碎小事（随口一提、天气评价、吃了什么、说了声谢谢）
- 0.1-0.2：几乎不值得记住（敷衍回应、无意义的闲聊 filler）
重要：如果用户只是随口闲聊（如评论天气、说"还行"、"哈哈"），不要提取为记忆。只提取有长期参考价值的信息。

如果某类没有值得记忆的内容，对应数组返回空数组 []。如果没有矛盾，corrections 返回空数组 []。
重要：无论对话内容是中文还是英文，所有提取的记忆内容（content）必须用中文书写。
只返回JSON，不要其他文字。"""
...
SELF_CONSOLIDATE_PROMPT_ZH = """以下是雨晴在不同对话中表达的关于自己的多条记忆，其中一些可能是相似或重复的。请合并为精炼的总结：
- 合并语义相近的记忆
- 保留所有独特信息
- 使用第一人称
- 简洁自然，不要啰嗦
...
_TEMPORAL_INTENT_KEYWORDS = [
    "之前", "以前", "上次", "那时候", "当时", "那天", "那天晚上",
    "记得", "还记得", "聊过", "说过", "提到", "告诉", "聊到",
    "什么时候", "哪天", "几时",
]


def parse_temporal_query(text: str) -> tuple[Optional[datetime], Optional[datetime]]:
    """Parse temporal references from user message.

    Returns (created_after, created_before) tuple.
    Both can be None — after is for lower bound, before is for upper bound.
    Returns (None, None) if no temporal intent detected.
    """
    text_lower = text.lower()

    has_intent = any(kw in text_lower for kw in _TEMPORAL_INTENT_KEYWORDS)

    now = datetime.utcnow()
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)

    # "今天" — only today's memories
    m = re.search(r"(今天|今日)", text_lower)
    if m and has_intent:
        return (today_start, None)

    # "昨天" — only yesterday (not today)
    m = re.search(r"(昨天|昨日)", text_lower)
    if m:
        yesterday_start = today_start - timedelta(days=1)
        return (yesterday_start, today_start)

    # "前天" — only the day before yesterday
    m = re.search(r"(前天)", text_lower)
    if m:
        day_before = today_start - timedelta(days=2)
        return (day_before, today_start - timedelta(days=1))

    # "这周" — start of this week (Monday) to now
    m = re.search(r"(这周|本周|这一个星期)", text_lower)
    if m and has_intent:
        days_since_monday = now.weekday()
        return ((now - timedelta(days=days_since_monday)).replace(hour=0, minute=0, second=0, microsecond=0), None)

    # "上周" — last Monday to this Monday
    m = re.search(r"(上周|上个星期)", text_lower)
    if m:
        days_since_monday = now.weekday()
    ...
    # "最近N天" — last N days to now (no upper bound)
    m = re.search(r"最近(\d+)[天日]", text_lower)
    if m:
        return (now - timedelta(days=int(m.group(1))), None)

    # "N天前" — that specific day only
    m = re.search(r"(\d+)\s*[天日]前", text_lower)
    if m:
        n = int(m.group(1))
        day_start = today_start - timedelta(days=n)
        return (day_start, day_start + timedelta(days=1))

    # "N周前" — that specific week
    m = re.search(r"(\d+)\s*[个]?(?:周|星期)前", text_lower)
    if m:
        n = int(m.group(1))
    ...
    # "N个月前" — approximate
    m = re.search(r"(\d+)\s*[个]?月前", text_lower)
    if m:
        n = int(m.group(1))
        start = now - timedelta(days=n * 30)
        return (start, None)

    # "这个月" — start of this month to now
    m = re.search(r"(这个月|本月)", text_lower)
    if m and has_intent:
        return (now.replace(day=1, hour=0, minute=0, second=0, microsecond=0), None)

    # "上个月" — last month
    m = re.search(r"(上个月)", text_lower)
    if m:
        first_this_month = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        last_day_prev = first_this_month - timedelta(days=1)
        first_prev_month = last_day_prev.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
...
def _time_ago(dt: datetime) -> str:
    """Format datetime with both absolute date and relative time for clarity."""
    now = datetime.utcnow()
    diff = now - dt
    days = diff.total_seconds() / 86400
    # Absolute date: M月D日 H:MM
    abs_str = f"{dt.month}月{dt.day}日 {dt.hour}:{dt.minute:02d}"
    if days < 1:
        return f"{abs_str}（今天）"
    elif days < 2:
        return f"{abs_str}（昨天）"
    elif days < 7:
        return f"{abs_str}（{int(days)}天前）"
    elif days < 30:
        return f"{abs_str}（{int(days / 7)}周前）"
    elif days < 365:
        return f"{abs_str}（{int(days / 30)}个月前）"
    else:
        return f"{abs_str}（{int(days / 365)}年前）"
...
class MemoryManager:
    ...
    async def build_context(
        ...
        # Parse temporal intent — if user asks about past events, filter by time range
        created_after, created_before = parse_temporal_query(user_message)
        is_temporal = bool(created_after or created_before)
        if is_temporal:
            logger.info(f"Temporal query detected, filtering: after={created_after}, before={created_before}")
            # For temporal queries, use user message directly for more precise semantic matching
            search_query = user_message
            # Strip temporal noise words from search query for cleaner BGE embedding
            _temporal_noise = ["上次", "还记得", "之前", "以前", "那时候", "当时", "那天", "那天晚上",
                              "记得", "聊过", "说过", "提到", "什么时候", "哪天", "几时",
                              "昨天", "今天", "前天", "上周", "这周", "最近"]
            cleaned = user_message
            for noise in _temporal_noise:
                cleaned = cleaned.replace(noise, "")
            cleaned = cleaned.strip()
            if len(cleaned) >= 4:
                search_query = cleaned

        # Dynamic top_k: temporal queries with narrow range need larger k (smaller candidate pool)
        if is_temporal and created_after and created_before:
            range_days = (created_before - created_after).total_seconds() / 86400
            top_k = settings.MEMORY_SEARCH_TEMPORAL_TOP_K if range_days <= 7 else 20
        else:
            top_k = 20
        ...
        # Build temporal context hint for the system prompt
        temporal_memory_hint = None
        if created_after or created_before:
            now = datetime.utcnow()
            today = now.replace(hour=0, minute=0, second=0, microsecond=0)
            if created_after and created_before:
                if created_before <= today and created_after >= today - timedelta(days=1):
                    temporal_memory_hint = f"用户问的是昨天的事，以下记忆的时间范围是 {created_after.month}月{created_after.day}日 到 {created_before.month}月{created_before.day}日，注意不要和今天混淆："
                elif created_after >= today:
                    temporal_memory_hint = f"用户问的是今天的事，以下记忆来自 {created_after.month}月{created_after.day}日 之后："
                else:
                    temporal_memory_hint = f"用户问的是过去的事，时间范围：{created_after.month}月{created_after.day}日 ~ {created_before.month}月{created_before.day}日："
            elif created_after:
                if created_after >= today:
                    temporal_memory_hint = f"用户问的是今天的事，以下记忆来自 {created_after.month}月{created_after.day}日 之后："
                elif created_after >= today - timedelta(days=1):
                    temporal_memory_hint = f"用户问的是昨天的事，以下记忆来自 {created_after.month}月{created_after.day}日 之后："
                else:
        ...
        # Attach temporal hint if time filtering was active
    ...
    async def _build_layered_memory(self, recalled: list, current_mood_warmth: float = 0.0,
                                      is_temporal_query: bool = False) -> dict:
        ...
        Args:
            ...
            current_mood_warmth: current mood warmth for mood-congruent recall bonus
            is_temporal_query: if True, sort facts/events by created_at (chronological) instead of hybrid score

        Returns:
            dict with keys: facts, events, episodic, behavior_rules, emotion_influences
            When is_temporal_query=True, also includes 'facts_by_date' dict grouped by date string
        """
        pool = await get_pool()
        layered = {
            "facts": [],
        ...
        def _recency_bonus(mem_id: str) -> float:
            """Bonus for recently created or accessed memories."""
            dt = created_at_map.get(mem_id)
            if not dt:
                return 0.0
            if isinstance(dt, datetime):
        ...
        if is_temporal_query and settings.MEMORY_TEMPORAL_ORDERED_INJECTION:
            # Collect all fact+event memories with their created_at datetime
            _all_timed = []
            for item in layered["facts"] + layered["events"]:
                mem_id = item.get("id")
                dt = created_at_map.get(mem_id) if mem_id else None
                if dt:
                    if isinstance(dt, datetime):
                        pass
                    else:
                        dt = datetime.fromisoformat(str(dt))
                    _all_timed.append((dt, item))
                else:
                    _all_timed.append((datetime.utcnow(), item))

            # Sort by time ascending (oldest first = chronological narrative)
            _all_timed.sort(key=lambda x: x[0])

            # Group by date string
            facts_by_date = {}
            for dt, item in _all_timed:
                date_key = f"{dt.month}月{dt.day}日"
                if date_key not in facts_by_date:
                    facts_by_date[date_key] = []
                # Use shorter time format for grouped display
                time_str = f"{dt.hour}:{dt.minute:02d}"
                facts_by_date[date_key].append({
                    "content": item["content"],
                    "time": time_str,
                })
            layered["facts_by_date"] = facts_by_date

            # Also re-sort facts and events chronologically (overwrite hybrid-score order)
            layered["facts"] = [item for _, item in _all_timed if item.get("memory_type") == "fact"]
            layered["events"] = [item for _, item in _all_timed if item not in layered["facts"]]
    ...
    async def _generate_inner_monologue(
        ...
        # Gather context for richer monologue
        today_context = ""
        if conversation_id:
            try:
        ...
        reflections_context = ""
        try:
            reflections = await self.get_self_reflections(limit=2)
            if reflections:
                reflections_context = "你之前的内心活动：\n" + "\n".join(
        ...
        prompt = prompt.replace("{today_context}", today_context)
        prompt = prompt.replace("{reflections_context}", reflections_context)
        prompt = prompt.replace("{mood_label}", mood_label)
        prompt = prompt.replace("{warmth}", str(warmth))
        prompt = prompt.replace("{openness}", str(openness))
    ...
    async def search_memories(self, query: str, top_k: int = 5,
                              ...
                              created_before: Optional[datetime] = None) -> list:
        """Search long-term memories by query using local bge embedding."""
        return await self._search_via_mysql(query, top_k, created_after=created_after, created_before=created_before)

    async def _search_via_mysql(self, query: str, top_k: int,
                                created_after: Optional[datetime] = None,
                                created_before: Optional[datetime] = None) -> list:
        """Use local embedding (bge-small-zh) for semantic search in MySQL."""
        pool = await get_pool()
