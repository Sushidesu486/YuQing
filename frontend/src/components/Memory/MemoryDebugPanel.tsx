...
const TYPE_COLORS: Record<string, string> = {
  ...
  episodic: 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300',
  emotion: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300',
  procedural: 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200',
};
...
function StatusBadge({ on, label }: { on: boolean; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${on ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
...
function ImportanceBar({ value }: { value: number | undefined }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400">{v.toFixed(2)}</span>
    </div>
  );
}
...
function ScoreBar({ label, value, color }: { label: string; value: number | undefined; color: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <span className="text-gray-400 dark:text-gray-500 dark:text-gray-400 w-8 text-right">{v.toFixed(3)}</span>
    </div>
  );
}
...
function OverviewTab({ onCleanup }: { onCleanup: (r: CleanupResult) => void }) {
  ...
  if (loading) return <div className="flex items-center justify-center py-16 text-sm text-gray-400 dark:text-gray-500 dark:text-gray-400">加载中...</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Type distribution */}
      <div>
        <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">类型分布 ({totalTypes})</div>
        <div className="space-y-1.5">
          {Object.entries(stats.by_type).map(([type, info]) => (
            <div key={type} className="flex items-center gap-2">
              <span className={`text-[10px] px-1.5 py-0.5 rounded ${TYPE_COLORS[type] || 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
                {type}
              </span>
              <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                <div className="h-full bg-blue-400 rounded-full" style={{ width: `${(info.count / maxCount) * 100}%` }} />
              </div>
              <span className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 w-6 text-right">{info.count}</span>
              <span className="text-[10px] text-gray-300 dark:text-gray-600 dark:text-gray-300">avg {info.avg_importance}</span>
            </div>
          ))}
          {Object.keys(stats.by_type).length === 0 && (
            <div className="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400 py-2">暂无记忆</div>
          )}
        </div>
      </div>
      ...
      <button
        ...
        disabled={cleaning}
        className="w-full py-2 text-xs font-medium rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800 disabled:opacity-50 transition"
      >
...
function StatCard({ label, value, isFloat, isText }: { label: string; value: string | number; isFloat?: boolean; isText?: boolean }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2.5 text-center">
      ...
      <div className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 mt-0.5">{label}</div>
    </div>
  );
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-2 pb-3">
        <div className="flex-1 relative">
          <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-gray-500 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input
            ...
            placeholder="搜索记忆内容..."
            className="w-full pl-8 pr-2 py-1.5 text-xs rounded-lg border border-gray-200 dark:border-gray-700 outline-none focus:border-blue-400 bg-gray-50 dark:bg-gray-800"
          />
        </div>
        <select
          value={typeFilter}
          onChange={e => setTypeFilter(e.target.value)}
          className="text-xs rounded-lg border border-gray-200 dark:border-gray-700 px-2 py-1.5 bg-gray-50 dark:bg-gray-800 outline-none focus:border-blue-400"
        >
          {TYPE_OPTIONS.map(t => (
      ...
      <div className="flex-1 overflow-y-auto -mx-1 space-y-1.5">
        {loading && <div className="py-8 text-center text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">加载中...</div>}
        {!loading && items.length === 0 && (
          <div className="py-8 text-center text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">暂无记忆</div>
        )}
        {items.map(item => (
          <div key={item.id} className="group relative p-2.5 rounded-lg border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
            <div className="flex items-start justify-between gap-2">
              <p className="text-xs text-gray-700 dark:text-gray-200 leading-relaxed flex-1 line-clamp-3">{item.content}</p>
              <button
                onClick={() => handleDelete(item.id)}
                className="shrink-0 p-1 rounded text-gray-300 dark:text-gray-600 dark:text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition"
                title="删除"
              >
            ...
            <div className="flex items-center gap-2 mt-1.5">
              ...
              {item.created_at && <span className="text-[10px] text-gray-300 dark:text-gray-600 dark:text-gray-300 ml-auto">{formatDate(item.created_at)}</span>}
            </div>
          </div>
        ))}
      </div>
...
function RecallDebugTab() {
  ...
  const stages = [
    ...
    { key: 'layered', label: '分层注入', color: 'bg-gray-50 dark:bg-gray-8000', count: 1 },
  ];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-2 pb-3">
        <input
          ...
          className="flex-1 px-3 py-2 text-xs rounded-lg border border-gray-200 dark:border-gray-700 outline-none focus:border-blue-400 bg-gray-50 dark:bg-gray-800"
        />
        <button
      ...
      {!result && !loading && (
        <div className="flex-1 flex items-center justify-center text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">
          输入消息后点击"测试"查看召回链路
        </div>
      )}

      {loading && (
        <div className="flex-1 flex items-center justify-center text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400">召回中...</div>
      )}

      {result && (
        <div className="flex-1 overflow-y-auto space-y-2">
          {/* Summary bar */}
          <div className="flex items-center gap-2 text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 px-1">
          ...
          {/* Stages accordion */}
          {stages.map(stage => (
            <div key={stage.key} className="border border-gray-100 dark:border-gray-800 rounded-lg overflow-hidden">
              <button
                onClick={() => toggle(stage.key)}
                className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800 transition"
              >
                <span className={`w-2 h-2 rounded-full ${stage.color} shrink-0`} />
                {stage.label}
                <span className="text-gray-400 dark:text-gray-500 dark:text-gray-400 font-normal">({stage.count})</span>
                <svg className={`w-3.5 h-3.5 ml-auto text-gray-400 dark:text-gray-500 dark:text-gray-400 transition-transform ${expanded[stage.key] ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="px-3 pb-3 border-t border-gray-50 dark:border-gray-800">
                  ...
                  {stage.key === 'spread' && (() => {
                    ...
                    return sp && sp.enabled ? (
                      <div className="mt-2 space-y-1">
                        <div className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400">
                    ...
                    ) : <div className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 py-2">未启用</div>;
                  })()}
                  ...
                  {stage.key === 'dormant' && (result.stage_dormant?.length ? result.stage_dormant.map((m, i) => (
                    <StageItem key={i} content={m.content} type={m.memory_type}>
                      ...
                      {m.dormant_days !== undefined && <span className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400">{m.dormant_days.toFixed(1)} 天</span>}
                    </StageItem>
                  )) : <EmptyStage />)}
                  ...
                  {stage.key === 'layered' && result.stage_layered && (
                    <div className="mt-2 space-y-2">
                      ...
                      {result.stage_layered.behavior_rules?.length > 0 && (
                        <div>
                          <div className="text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">Behavior Rules</div>
                          {result.stage_layered.behavior_rules.map((r, i) => (
                            <div key={i} className="text-xs text-gray-600 dark:text-gray-300 pl-2 border-l-2 border-gray-200 dark:border-gray-700 mb-1">{r}</div>
                          ))}
                        </div>
                      )}
                      {result.stage_layered.emotion_influences?.length > 0 && (
                        <div>
                          <div className="text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">Emotion Influences</div>
                          {result.stage_layered.emotion_influences.map((e, i) => (
                            <div key={i} className="text-xs text-gray-600 dark:text-gray-300 pl-2 border-l-2 border-red-200 mb-1">
  ...
  return (
    <div className="mt-2 p-2 rounded bg-gray-50 dark:bg-gray-800">
      <div className="flex items-start gap-1.5">
        ...
        <p className="text-xs text-gray-700 dark:text-gray-200 leading-relaxed line-clamp-3">{content}</p>
      </div>
...
function EmptyStage() {
  return <div className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 py-2">无匹配记忆</div>;
}
...
function LayeredSection({ title, items }: { title: string; items: Array<{ content: string; id?: string; memory_type?: string; created_at_relative?: string }> }) {
  ...
  return (
    <div>
      <div className="text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1">{title} ({items.length})</div>
      {items.map((item, i) => (
        <div key={item.id ?? i} className="text-xs text-gray-600 dark:text-gray-300 pl-2 border-l-2 border-gray-200 dark:border-gray-700 mb-1">
          {item.content}
          {item.created_at_relative && <span className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-400 ml-1">({item.created_at_relative})</span>}
        </div>
      ))}
    </div>
  );
...
function GraphTab() {
  ...
  const isDark = localStorage.getItem('yuqing_dark_mode') === '1';
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 mb-2">
        <div className="flex-1 relative">
          <svg className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-400 dark:text-gray-500 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <input
            type="text"
            ...
            className="w-full pl-7 pr-2 py-1 text-xs rounded border border-gray-200 dark:border-gray-700 outline-none focus:border-blue-400 bg-gray-50 dark:bg-gray-800"
          />
        </div>
        <div className="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-400 shrink-0">
      ...
      <div ref={containerRef} className="flex-1 relative">
        <ForceGraph2D
          ...
          nodeCanvasObject={(node, ctx, globalScale) => {
            ...
            ctx.fillStyle = isDark ? 'rgba(31,41,55,0.85)' : 'rgba(255,255,255,0.8)';
            ctx.beginPath();
            ctx.roundRect(
...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400 dark:text-gray-500 dark:text-gray-400 gap-2">
...
export function MemoryDebugPanel({ open, onClose }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-30 bg-white dark:bg-gray-900 flex flex-col" style={{ animation: 'slideIn 0.2s ease-out' }}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          <h2 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Memory Debug</h2>
          {cleanupResult && (
            <span className="text-[10px] text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-1.5 py-0.5 rounded">
              清理完成: 合并 {cleanupResult.clusters_merged}
            </span>
          )}
        </div>
        <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 dark:bg-gray-700 text-gray-400 dark:text-gray-500 dark:text-gray-400">
