...
- [x] 被动检索结果实时注入 messages context，雨晴可在回复中引用
...
- [x] AGENTS.md：OpenCode agent 使用指南

### 近期新增（2026-05-08 ~ 2026-05-09）
- [x] System prompt 拆分为 stable/dynamic 双模板（前缀 cache 优化）
- [x] 情绪轨迹 + 跨会话情绪画像（Phase 2.1/2.15）
- [x] `search_knowledge` 工具（搜索本地 knowledge_items）
- [x] 人格 prompt 精简：speech_patterns 16→6 条，emotion_response 6→3 条
- [x] 关系阶段动态选择：用 days_known 只注入当前阶段
- [x] defense_mechanisms 模板平铺（不再 dict 循环）
- [x] 贴纸格式修复：历史 sticker 改为 /name 格式注入上下文
- [x] S1 语义链接 + S2 动态链接强度（recall_count / last_recalled）
- [x] 内部独白系统（Phase 8.5）：fire-and-forget，生成 self_reflection 记忆
- [x] 今日对话全记录：动态 prompt 顶部注入全天对话日志（40字/条, 50轮）
- [x] 今日话题防重复：加载今天 assistant 回复，提示不要重复主动聊天话题
- [x] 召回 touch_memory 覆盖 episodic + emotion（修复无限衰减）
...
1. **搜索 query 太泛**：从 YAML interests 提取 topic 后只拼接"最新资讯"，如"ACG 文化 最新资讯"、"历史和神话 最新资讯"，返回的是大路货新闻，不符合雨晴人设。雨晴的兴趣描述中有具体方向（"老玩家品味洁癖"、"高性能计算"、"后朋克和摇滚"），完全没利用。

2. **总结 prompt 返回"感想"而非"事实"**：当前 prompt 要求"以雨晴的第一人称视角，像是她看到了这些信息后的感想"。实际存入 knowledge_items 的全是"我觉得…真不错"、"这脑洞也太大了"之类的感叹，没有任何实际信息量。7 天内可引用，但对对话毫无价值——雨晴不可能只说一句"最近有个事挺有意思"但不提是什么事。

3. **LLM 总结时无人格上下文**：`generate_completion(messages=[{"role": "user", "content": prompt}])` 没有传 system prompt，导致：
   - 总结语气不像雨晴（"真是让人又期待又着急呀"这种不像她会说的话）
   - 没有利用雨晴的性格特质（吐槽、调侃、毒舌风格）来过滤/评价信息
...
雨晴对时间的感知几乎是"盲"的。当前代码中只有零散的时间相关逻辑，没有统一的时间上下文模块：
...
**核心缺失**：
1. 雨晴不知道"我和这个人认识多久了"（关系任期）
2. 雨晴不知道"现在是深夜还是下午"（时段感知不影响回复风格）
3. 雨晴不知道"用户刚走开5分钟还是消失了3天"（会话间隔语义）
4. 雨晴不知道"这轮对话已经聊了2小时"（对话时长感知）
...
| 时段感知 | ⭐⭐⭐⭐⭐ | 纯计算。`datetime.now().hour` 映射到时段描述，注入 prompt 让雨晴知道"现在是深夜/下午"。低风险高回报。 |
| 关系任期 | ⭐⭐⭐⭐⭐ | `conversations.created_at` 或最早消息时间 → 计算认识天数。纯查询，无风险。让雨晴能说"我们认识快一个月了"。 |
| 对话时长感知 | ⭐⭐⭐⭐ | 当前会话的消息跨度。可能影响回复节奏（聊久了可以更放松）。低风险。 |
| 时间锚定记忆 | ⭐⭐⭐⭐ | 记忆的 created_at 在查询时转为相对时间描述（"上周"、"昨天"、"去年"）。纯格式化，无风险。 |
| 昼夜节律 | ⭐⭐⭐ | 深夜雨晴应该更安静/迷糊，白天更活跃。中度风险（过度表演），需要微妙编码。 |
...
1. **回复自然度提升**：雨晴能根据时段调整语气（深夜简短、白天正常），根据间隔调整开场（"刚走？" vs "好久不见"），根据认识天数调整亲密程度暗示
2. **记忆召回增强**：时间锚定记忆让雨晴能引用具体时间（"你上个月说的那个项目"），比纯内容引用更真实
...
4. **时间锚定记忆召回**（Phase 3.6 情感真实性的 mood-congruent recall 前置）：
   ...
   - LLM prompt 中已有 `recalled_memories`，但雨晴无法说"你去年说的那个"因为不知道记忆的时间
...
### 3.5 雨晴自我认知深化（SelfCognitionEngine）
